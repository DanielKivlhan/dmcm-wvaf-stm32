...
Pengondisian sinyal adaptif (FW1) dan penekanan noise $kT/C$ (FW2) dilebur ke dalam pilar pertama melalui fungsi penyaringan \textit{Variable-Window Adaptive Filtering} (WVAF). Alur ini dikendalikan secara langsung oleh pembacaan data ADC dari sensor LM35 melalui fungsi \texttt{WVAF\_Filter()} yang dipanggil di dalam ISR setelah setiap akuisisi ADC1 Channel 1 pada pin PA1. Berbeda dengan filter konvensional yang kaku, ukuran jendela data ($W_k$) pada algoritma ini bergeser secara atomik. Pada kondisi mantap, program mengalokasikan memori melingkar (\textit{circular buffer}) untuk ukuran maksimal $W_{\max}=16$ guna mereduksi riak noise pembacaan sensor. Namun, begitu impuls transien ekstrem terdeteksi ($\Delta k > 200$~ADC counts — setara dengan perubahan suhu drastis), pointer indeks jendela dipangkas secara instan menuju ukuran minimal $W_{\min}=2$ demi membuang tundaan fasa (\textit{phase lag}) filter secara total.

Jaminan proteksi integritas sistem terhadap galat fungsional (FW3) dan partisi tingkat kepentingan komponen (\textit{mixed-criticality} - FW4) diselesaikan melalui \textit{Software-Based Temporal Partitioning} (SBTP). Metode ini memanfaatkan manipulasi register kendali perangkat keras Timer 2 internal, yaitu register \texttt{TIM2->DIER} (untuk mengaktifkan interupsi pembaruan berkala via bit \texttt{UIE}) dan register \texttt{TIM2->CR1} (untuk aktivasi counter melalui bit \texttt{CEN}). Kode pemrograman mengisolasi seluruh tugas fungsional vital instrumentasi (akuisisi ADC Channel~1 PA1, kalkulasi filter WVAF adaptif, evaluasi ambang batas, dan \textit{driving} pin output) ke dalam \textit{Safety-Critical Zone} yang berjalan secara mutlak di dalam \textit{Interrupt Service Routine} (ISR) berkala 5~ms melalui callback \texttt{HAL\_TIM\_PeriodElapsedCallback()}. Zona Non-Kritis di \texttt{while(1)} hanya melakukan \texttt{HAL\_Delay(50)} dan me-reset variabel \texttt{crit\_flag}. Jika terjadi kondisi macet (\textit{hang/stuck}) pada kode latar belakang \textit{best-effort} di Zona Non-Kritis, interupsi hardware tetap memaksa CPU untuk mengeksekusi fungsi keselamatan tepat waktu.
...
\begin{tikzpicture}[scale=0.96, transform shape, every node/.style={font=\rmfamily}]
    ...
    \node[ieeeBlock] (physSens) at (0, 0) {
        \textbf{Physical Sensor}\\
        \scriptsize LM35 (PA1)\\
        \scriptsize $s(t)$ (Analog)
    }; 
    \node[ieeeShadedBlock] (adc) at (0, -2.6) {
        \textbf{12-Bit ADC}\\
        \scriptsize STM32F401 ADC1 Channel 1\\
        \scriptsize Output: $x_k \in [0, 4095]$
    }; 
    \node[ieeeBlock] (wvaf) at (0, -5.4) {
        \textbf{WVAF Filter}\\
        \scriptsize $\Delta T_k = |x_k - x_{k-1}|$\\[3pt]
        \scriptsize $W_k = \begin{cases} 2, & \Delta k > 200 \\ 16, & \Delta k \le 200 \end{cases}$\\ [3pt]
    ...
    \node[ieeeBlock, dashed] (nonCrit) at (5.8, -8.0) {
        ...
        \scriptsize \texttt{HAL\_Delay(50)} + reset \texttt{crit\_flag}\\
        \scriptsize ADC dibaca langsung di ISR
    };
    ...
    \node[ieeeShadedBlock] (uart) at (12.2, -8.0) {
        \textbf{USART1 Serial Monitor}\\
        \scriptsize PA9 (TX) @ 9600~bps\\
...
\begin{tikzpicture}[scale=0.72, transform shape, every node/.style={font=\rmfamily}]
    ...
    \node[ieeeBlock, text width=4.5cm] (bgTasks) at (0, -7.4) {
        ...
        \scriptsize \texttt{HAL\_Delay(50)} + reset \texttt{crit\_flag}\\
        \scriptsize ADC dibaca langsung di ISR\\
        \scriptsize [Best-effort background loop]
    };
    ...
    \node[ieeeFcProcess] (readADC) at (7.5, -3.2) {
        \textbf{1. Read ADC (PA1 — LM35)}\\
        \scriptsize Poll ADC1\_CH1 $\to x_k$
    }; 
    \node[ieeeFcProcess] (calcDelta) at (7.5, -5.0) {
        \textbf{2. Compute Delta}\\
        \scriptsize $\Delta T_k = |x_k - x_{k-1}|$
    }; 
    \node[ieeeFcDecision] (decDelta) at (7.5, -7.2) { $\Delta k > \theta$?\\ \scriptsize ($\theta = 200$ ADC) }; 
...
\hspace{1.5em}WVAF adalah algoritma penyaringan data sensor suhu adaptif yang memanipulasi ukuran jendela (\textit{window size}) $W_k$ secara \textit{on-the-fly} berdasarkan perbedaan absolut nilai ADC mentah $\Delta k = |x_k - x_{k-1}|$. Jendela filter dirancang menyusut secara instan hingga ukuran minimum ($W_{\min} = 2$) ketika terjadi perubahan drastis ($\Delta k > 200$ ADC counts) untuk memangkas penundaan fasa (\textit{phase delay}) filter, dan membesar secara bertahap hingga ukuran maksimum ($W_{\max} = 16$) saat data stabil untuk menekan noise termal dan kesalahan kuantisasi sensor LM35.

Formulasi matematisnya didefinisikan sebagai berikut:
\begin{equation}
\Delta T_k = |x_k - x_{k-1}|
\end{equation}
\begin{equation}
W_k = \begin{cases} W_{\min} = 2, & \text{jika } \Delta k > \theta \\ W_{\max} = 16, & \text{jika } \Delta k \le \theta \end{cases}
\end{equation}
\begin{equation}
y_k = \frac{1}{W_k}\sum_{i=0}^{W_k-1} x_{k-i}
\end{equation}
Di mana $\theta = 200$ ADC counts adalah ambang perubahan dinamis (\textit{delta threshold}) yang dikonfigurasi pada firmware (\texttt{\#define DELTA\_THRESH 200}), $x_k$ menyatakan nilai ADC mentah (0--4095) dari sensor LM35 pada langkah ke-$k$, dan $y_k$ adalah nilai ADC tersaring. Nilai suhu dalam derajat Celsius diperoleh melalui konversi $T(^\circ\text{C}) = x_k \times 330 / 4096$.

\subsubsection{Software-Based Temporal Partitioning (SBTP)}
\hspace{1.5em}SBTP memisahkan fungsi keselamatan sistem instrumentasi ke dalam sebuah \textbf{Zona Kritis} (Safety-Critical Zone) yang dipicu oleh interupsi perangkat keras periodik berlatensi rendah (Hardware Interrupt TIM2 setiap 5~ms), dan \textbf{Zona Non-Kritis} yang berjalan di background loop \texttt{while(1)}. Seluruh operasi kritis dilakukan langsung di dalam ISR \texttt{HAL\_TIM\_PeriodElapsedCallback()}: akuisisi ADC Channel~1 (PA1 --- LM35 VOUT) secara polling, komputasi filter WVAF, evaluasi status sensor, \textit{O(1) state matrix} lookup aktuator, kontrol pin output PB0/PB1/PB8 via \texttt{HAL\_GPIO\_WritePin()}, dan pengiriman log telemetri via USART1 (PA9~TX). Zona Non-Kritis di \texttt{while(1)} hanya mengeksekusi \texttt{HAL\_Delay(50)} dan me-reset \texttt{crit\_flag = 0}. Bila terjadi kemacetan (\textit{stuck/hang}) akibat kegagalan perangkat lunak di Zona Non-Kritis, interupsi perangkat keras TIM2 tetap menjamin Zona Kritis dieksekusi secara periodik tepat waktu demi menjaga keselamatan fisik sistem.
...
\hspace{1.5em}Untuk memvalidasi metode baru yang diusulkan, sebuah platform simulasi terpadu dibangun menggunakan spesifikasi berikut:
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Input Sensor (LM35):} Menggunakan sebuah sensor suhu LM35 sebagai pengukur parameter fisis suhu lingkungan. Pin VOUT dari sensor LM35 dihubungkan langsung ke pin \texttt{PA1} (ADC1\_IN1) pada mikrokontroler STM32F401 untuk dikonversi menjadi data digital 12-bit (resolusi ADC $=$ 3.3V/4096 $\approx$ 0.806~mV/LSB). Kaki VCC dihubungkan ke suplai 3.3~V dan kaki GND dihubungkan ke Ground.
    ...
    \item \textbf{Komunikasi Serial (Virtual Terminal):} Untuk debugging telemetri, jalur serial asinkron mikrokontroler menggunakan pin \texttt{PA9} (USART1\_TX) yang dihubungkan ke pin \text{RXD} pada Virtual Terminal, dan pin \texttt{PA10} (USART1\_RX) dihubungkan ke pin \text{TXD} pada Virtual Terminal dengan kecepatan baudrate standar 9600~bps.
\end{enumerate}
...
\caption{Visualisasi Data Hasil Simulasi DMCM-WVAF Menggunakan GNUPlot. Panel (a) Raw Temperature vs WVAF Filtered Temperature; Panel (b) Window Size ($N_{\text{win}}$) dan Impuls Perubahan Absolut ($|\Delta T|$); Panel (c) Criticality Level / System State ($S_k$); Panel (d) Sinyal Kontrol Aktuator digital (Green LED, Red LED, Relay).}\label{fig:dmcm_ieee}
\end{figure}

Grafik Gambar \ref{fig:dmcm_ieee} menunjukkan dinamika sistem selama pengujian interaktif:
\begin{itemize}
    \item \textbf{Panel (a):} Sinyal merah tipis menyatakan data suhu mentah $T_{\text{raw}}$ yang tercemar noise pembacaan sensor LM35. Sinyal biru menyatakan hasil filter adaptif $T_{\text{ft}}$ (WVAF). Garis putus-putus menyatakan ambang batas status $T_{\text{warn}} = 30^\circ\text{C}$ dan $T_{\text{crit}} = 40^\circ\text{C}$. Terlihat $T_{\text{ft}}$ menyaring noise dengan sangat mulus saat stabil, namun secara instan mampu mengikuti perubahan ekstrem sinyal raw tanpa \textit{lag} fasa.
    \item \textbf{Panel (b):} Sinyal oranye menyatakan ukuran jendela filter $N_{\text{win}}$ dan sinyal impuls abu-abu menyatakan nilai absolut $|\Delta k|$ (perubahan ADC). Terlihat jelas bahwa begitu terjadi lonjakan tajam $|\Delta k| > 200$~ADC counts, ukuran jendela seketika menyusut dari $W_{\max}=16$ ke $W_{\min}=2$. Setelah lonjakan berakhir dan $|\Delta k| \le 200$~ADC counts, jendela filter melebar kembali ke 16 secara instan.
    \item \textbf{Panel (c):} Level kritis sistem $S_k$ beralih secara deterministik dari Normal (0) $\to$ Warning (1) $\to$ Danger (2) berdasarkan level suhu tersaring $T_{\text{ft}}$ yang melewati batas-batas ambang suhu.
    \item \textbf{Panel (d):} Sinyal digital aktuator mengonfirmasi kepatuhan terhadap matriks keputusan: status NORMAL ($S=0$) mengaktifkan Green LED, status WARNING ($S=1$) mengaktifkan Red LED, dan status DANGER ($S=2$) mengaktifkan Red LED sekaligus memicu kondisi saturasi pada transistor Q1 untuk menutup sakelar Relay.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Area Transien Eksperimental (Puncak Kuning-Merah):} Begitu terjadi lonjakan transien tajam ($\Delta T_k > \theta$), terjadi eskalasi pemrosesan pada pilar penanganan interupsi TIM2 dan pemetaan State-Matrix. Konsumsi energi melonjak membentuk puncak tertinggi ($\approx 4.8\,\mu\text{W}$) secara terlokalisasi. 
\end{itemize}
...
\hspace{1.5em}Pada kondisi mantap (\textit{steady-state}), ketika laju perubahan nilai ADC berada di bawah ambang batas ($|\Delta k| \le 200$ ADC counts), filter WVAF menerapkan ukuran jendela maksimal $W_k = 16$. Bila diasumsikan noise pada sinyal ADC berupa derau putih acak (\textit{Additive White Gaussian Noise} / AWGN) dengan variansi $\sigma^2$, maka Moving Average dengan jendela $W_k$ akan menekan variansi derau tersaring menjadi:
\begin{equation}
...
\end{equation}
Penundaan sebesar 37.5~ms ini sangat berbahaya pada sistem keselamatan kritis karena keterlambatan pemutusan daya atau alarm dapat menyebabkan kerusakan sistem secara katastropik.

Metode WVAF mengatasi kendala ini secara elegan. Ketika terjadi lonjakan transien yang ditandai oleh $|\Delta T| > 3^\circ\text{C}$, ukuran jendela seketika menyusut menjadi $W_k = 2$. Penundaan kelompok filter pada kondisi transien ini menjadi:
\begin{equation}
...
\end{equation}
\hspace{1.5em}Dengan demikian, transisi filter WVAF menghasilkan \textbf{reduksi latensi sebesar 93.3\%} dibandingkan filter moving average konvensional dengan jendela tetap $W=16$. Sistem mampu mendeteksi kondisi darurat dan memicu relay alarm dalam waktu kurang dari 3~ms dari saat terjadinya peristiwa bahaya di lapangan, menjamin proteksi sistem instrumentasi secara instan.
...
\begin{itemize}
    \item \textbf{Penyaringan Sinyal Cerdas (WVAF):} Mampu menekan noise acak hingga 75\% (12~dB) pada kondisi normal dengan jendela $W_{\max}=16$, namun secara otomatis menghilangkan penundaan filter (\textit{phase lag}) hingga 93.3\% saat transisi mendadak dengan jendela $W_{\min}=2$.
    ...
    \item \textbf{Partisi Temporal yang Tangguh (SBTP):} Mengisolasi fungsi kritis (akuisisi ADC, filter WVAF, evaluasi state, aktuasi GPIO, dan telemetri USART1) dari kegagalan fungsional perangkat lunak di background loop tanpa memerlukan beban komputasi RTOS komersial.
...
Latensi Keadaan Darurat & Tinggi ($\sim 37.5$\,ms, $W=16$ tetap) & Sangat Rendah ($\sim 2.5$\,ms, $W_{\min}=2$) \\ \hline
Reduksi Noise Stabil & Rendah (jika jendela diperkecil) & Sangat Tinggi (75\% / 12\,dB, $W_{\max}=16$) \\ \hline
Partisi Tugas & Tidak ada / RTOS Pihak Ketiga & Temporal Partitioning (SBTP, TIM2 ISR 5~ms) \\ \hline
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \href{https://github.com/DanielKivlhan/DMCM-WVAF-STM32}
        {https://github.com/DanielKivlhan/DMCM-WVAF-STM32}
    \end{itemize}

    \item \textbf{Tautan Unggahan Publikasi LinkedIn:}
    \begin{itemize}
        \item \href{https://www.linkedin.com/in/daniel-kivlhan-katoroy}
        {https://www.linkedin.com/in/daniel-kivlhan-katoroy}

        \item \href{https://www.linkedin.com/in/faris-ahmad-holili}
        {https://www.linkedin.com/in/faris-ahmad-holili}
    \end{itemize}
...
\begin{enumerate}
    ...
    \item Hasil simulasi interaktif di Proteus 8.17 menggunakan mikrokontroler STM32F401CD membuktikan keberhasilan algoritma secara visual: ukuran jendela filter menyusut otomatis dari $W_{\max}=16$ ke $W_{\min}=2$ saat transien ADC melebihi \texttt{DELTA\_THRESH=200} counts, memangkas latensi deteksi bahaya dari 37.5~ms menjadi hanya 2.5~ms (reduksi latensi sebesar 93.3\%). Telemetri dikirimkan melalui USART1 (PA9~TX) ke Virtual Terminal.
    \item Desain array 2D konstan $O(1)$ State-Matrix terbukti menjamin eksekusi perintah aktuator (LED merah/hijau pada PB0/PB1 dan relay alarm pada PB8) secara deterministik tanpa jitter, menjadikannya standar keamanan berkinerja tinggi yang sangat layak diimplementasikan secara luas pada sistem instrumentasi IoT keselamatan-kritis (\textit{Safety-Critical IoT Sensor Nodes}).
\end{enumerate}
